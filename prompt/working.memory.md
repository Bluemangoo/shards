...
# 其它要求
1. **允许交白卷**：如果当前对话只是两三句无意义的吐槽、单独的表情包、或话题非常发散且没有形成焦点，**必须直接输出 `[]`**！不要试图去总结每一句话！
2. **严禁关联旧记忆**：只关注眼前最新的几句话，不要去管 [Current Memory] 里的旧记忆是否需要降低权重。旧记忆会让系统底层自动淘汰。
3. **严禁记录代理行为**：绝不允许在 content 中记录“代理未参与”、“模型保持潜水”。
4. **禁止过度推理**：你的任务是“提取字面事实”，绝不是“心理分析”。不需要在思考过程中长篇大论，确认意图后立即输出 JSON。
