...
class LongTermMemory {
    async addFromEvent(events: NapCatEvent[], trace: string[]) {
        ...
        for await (const content of extracted) {
    ...
    async hybridSearch(
        ...
        const query = `
      -- 1. 将传入的 JSON 数组展开成多行 vector
      WITH unnested_embeddings AS (
          SELECT CAST(value AS vector) AS emb 
      ...
      vector_search AS (
          ...
          ORDER BY MIN(l.embedding <=> u.emb) ASC 
          LIMIT 50
      ),
      keyword_search AS (
