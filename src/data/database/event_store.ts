...
export class EventStore {
    static async push_event(event: any): Promise<void> {
    ...
    static async get_window_events(
        ...
        for (let i = rows.length - 1; i >= 0; i--) {
            events.push(rows[i].data);
        }
        return events;
    }

    static async get_distinct_message_events(count: number = 10): Promise<NapCatEvent[]> {
        const query = `
      SELECT id, data FROM (
        SELECT DISTINCT ON (data->>'target_id') id, data 
        FROM events 
        WHERE data->>'message_id' IS NOT NULL AND data->>'target_id' IS NOT NULL 
        ORDER BY data->>'target_id'
