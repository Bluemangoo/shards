...
2. 缓冲层: EventStack 会根据消息数量和时间延迟进行批处理。当达到触发条件（默认5条/60s）或被显式唤醒时（例如被艾特），将一组消息发送给生命周期管理器。
3. 主处理循环：
...
代码中混乱的命名规则(驼峰和下划线)一部分是为了匹配 napcat 的，另一部分情况是因为最开始是在 python 写的，迁过来的时候没改。ts 我还是喜欢大驼峰。
